...
let test_KSBetween () =
    ...
    let b = Deliberation_model.Distances.ksBetween in
        check bool
          "Testing if (a = b = c) is not between a > b > c and c > b > a" false
          (b p p2 p_eq);
        check bool
          "Testing if (a = b = c) is not between a > b > c and b > c > a" false
          (b p2 p3 p_eq);
        check bool
          "Testing if (a = b = c) is not between a > b > c and b > c > a" true
          (b p p4 p_eq)
...
let test_deliberation () =
    ...
    let voters =
        List.map2
          (fun p b ->
            { Utils.preference = p; Utils.bias = b; Utils.announced = 0 })
          profile biases
    in
    let updated =
...
let test_transitive () =
    ...
    let result_transative = Evaluations.is_transitive profile_transative in
        check bool
          "Correctly determines if intransative profile is not transative" false
          result_intransative;
        check bool "Correctly determines if transative profile is transative"
          true result_transative

let test_condorcet () =
    let profile_noncondorcet =
        [
          [ [ 1 ]; [ 2 ]; [ 3 ] ];
          [ [ 3 ]; [ 1 ]; [ 2 ] ];
          [ [ 2 ]; [ 3 ]; [ 1 ] ];
    ...
    in
    let result_nocondorcet = Evaluations.has_condorcet profile_noncondorcet in
    let result_condorcet = Evaluations.has_condorcet profile_condorcet in
        check bool "Correctly determines when a profile has no condorcet winner"
          false result_nocondorcet;
        check bool "Correctly determines when a profile has a condorcet winner"
          true result_condorcet
...
let test_cyclic () =
    ...
    in
    let result_cyclic = Evaluations.is_cyclic profile_cyclic in
    let result_noncyclic = Evaluations.is_cyclic profile_noncyclic in
        check bool "Correctly determines if cyclic profile is cylic " true
          result_cyclic;
        check bool "Correctly determines if acyclic profile is acyclic" false
          result_noncyclic
...
let () =
    run "Deliberation model Testing"
      [
        ( "is_single_peaked",
          [
            test_case "Check single-peaked profiles" `Quick
              test_is_single_peaked;
          ] );
        ( "dpDistance",
        ...
        ( "Condorcet",
          [ test_case "Check Condorcet winner detection" `Quick test_condorcet ]
        );
        ( "Cyclic",
          [ test_case "Check Cyclic profile detection" `Quick test_cyclic ] );
